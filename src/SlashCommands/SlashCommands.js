...
class SlashCommands {
  constructor (client, config) {
    ...
    if (config.UNIT_TESTS) { return }

    this.FillRegisteredCommands()

    /// Read from the global and local directory to determine local commands.
    ReadDirectory(path.join(__dirname, '../../commands/global'), Resolved => { const command = new Resolved(this.client); this.COMMANDS.GLOBAL.set(command.name, command) })
    ReadDirectory(path.join(__dirname, '../../commands/private'), Resolved => { const command = new Resolved(this.client); this.COMMANDS.LOCAL.set(command.name, command) })
  ...
  async Purge () {
    this.REGISTERED_COMMANDS.LOCAL.forEach(command => {
      if (!this.COMMANDS.LOCAL.has(command.name)) { this.Remove({ id: command.id, local: true }) }
    })

    this.REGISTERED_COMMANDS.GLOBAL.forEach(command => {
      if (!this.COMMANDS.GLOBAL.has(command.name)) { this.Remove({ id: command.id, local: false }) }
    })
  }
